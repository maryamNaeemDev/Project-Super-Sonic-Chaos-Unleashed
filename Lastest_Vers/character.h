...
public:
	...
	void setVelocityLimit(int b) { velocitylimit = b; }
	int getVelocityLimit() { return velocitylimit; }

	bool getPower() { return powerup; }
	void setPower(bool b) { powerup = b; }
	bool getCantHurtMe() { return canthurtme; }

	float getVelocityX() { return velocityX; }
	float getAcceleration() { return acceleration; }
	void setVelocityX(float f) { velocityX = f; }
	void setAcceleration(float f) { acceleration = f; }
	...
	void player_gravity(char** lvl, float& offset_y, float& gravity, float& terminal_Velocity, int& hit_box_factor_x, int& hit_box_factor_y, const int cell_size, int& Pheight, int& Pwidth)
	{
		...
		if (velocityY < 0) // Only check when moving upward (jumping)
		{
			char top_left = lvl[(int)(player_y) / cell_size][(int)(player_x + hit_box_factor_x) / cell_size];
			char top_right = lvl[(int)(player_y) / cell_size][(int)(player_x + Pwidth - hit_box_factor_x) / cell_size];
			char top_center = lvl[(int)(player_y) / cell_size][(int)(player_x + Pwidth / 2) / cell_size];

			if (top_left == 'w' || top_left == 'q' || top_left == 'e' || top_left == 'b' ||
				top_left == 'z' || top_left == 'p'  ||
				top_right == 'w' || top_right == 'q' || top_right == 'e' || top_right == 'b' ||
				top_right == 'z' || top_right == 'p' ||
				top_center == 'w' || top_center == 'q' || top_center == 'e' || top_center == 'b' ||
				top_center == 'z' || top_center == 'p')
			{
				// Hit a block from below - stop upward movement
				velocityY = 0;
				// Position player just below the block
				int blockBottomY = ((int)(player_y) / cell_size + 1) * cell_size;
				player_y = blockBottomY;
			}
		}
		...
		if (bottom_left == 'w' || bottom_left == 'q' || bottom_left == 'e' || bottom_left == 'b' || bottom_left == 'z' || bottom_left == 'p' || bottom_left == 'j') groundPoints++;
		if (bottom_left == 'w' || bottom_left == 'q' || bottom_left == 'e' || bottom_left == 'b' || bottom_left == 'z' || bottom_left == 'p' || bottom_left == 'j') groundPoints++;
		if (bottom_left == 'w' || bottom_left == 'q' || bottom_left == 'e' || bottom_left == 'b' || bottom_left == 'z' || bottom_left == 'p' || bottom_left == 'j') groundPoints++;
	...
	void move_right(char** lvl, const int cell_size, const int width)
	{
		...
		int gridX = static_cast<int>(player_x) / cell_size;
		int gridY = static_cast<int>(player_y) / cell_size;
		if (gridX < width - 1 && (lvl[gridY + 1][gridX + 2] != 'w' && lvl[gridY + 1][gridX + 2] != 'q' && lvl[gridY + 1][gridX + 2] != 'b' && lvl[gridY + 1][gridX + 2] != 'e'))
		{
	...
	void move_left(char** lvl, const int cell_size, const int width)
	{
		...
		int gridX = static_cast<int>(player_x) / cell_size;
		int gridY = static_cast<int>(player_y) / cell_size;
		if (gridX > 0 && lvl[gridY + 1][gridX] != 'w' && lvl[gridY + 1][gridX] != 'q' && lvl[gridY + 1][gridX] != 'b' && lvl[gridY + 1][gridX] != 'e')
		{
			if (velocityX > - velocitylimit)
			{
				velocityX -= acceleration;
...
public:
	...
	void power() override
	{
		if (powerup && empowering.getElapsedTime().asSeconds() < 20)
		{
			acceleration = 1.5;
			velocitylimit = 12;
		}
	...
	void breakwall(char** lvl, float& offset_y, float& gravity, float& terminal_Velocity, int& hit_box_factor_x, int& hit_box_factor_y, const int cell_size, int& Pheight, int& Pwidth)
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::B))
		{
			...
			if (breakwallclk.getElapsedTime().asSeconds() > 2)
			{
				...
				if ((lvl[gridY][gridX] == 'b' || lvl[gridY + 1][gridX] == 'b' || lvl[gridY][gridX] == 'w' || lvl[gridY + 1][gridX] == 'w') && bruh)
				{
				...
				else if ((lvl[gridY][gridX + 2] == 'b' || lvl[gridY + 1][gridX + 2] == 'b' || lvl[gridY][gridX+2] == 'w' || lvl[gridY + 1][gridX+2] == 'w') && !bruh)
				{
					cout << "HERERERERERERE" << endl;
					lvl[gridY][gridX + 2] = 's';
...
public:
	...
	void fly(char** lvl, int& hit_box_factor_x, int& hit_box_factor_y, const int cell_size, int& Pheight, int& Pwidth)
	{
		int gridX = static_cast<int>(player_x) / cell_size;
		int gridY = static_cast<int>(player_y) / cell_size;
...
public:
	...
	Player(Characters* yoo)
	{
		...
		if (!jumpBuffer.loadFromFile("Data/Jump.ogg")) {
	...
	bool getIsJumping() { return active->getIsJumping(); }
	void setscore(int s) { score = s; }
	bool getHurt() { return hurt; }
	void setHurt(bool h) { hurt = h; }
	bool getHurtRight() { return hurtright; }
	void setHurtRight(bool h) { hurtright = h; }
	...
	void setPowerUp(bool b)
	{
		active->setPower(b);
	}
	bool getPowerUp()
	{
		return active->getPower();
	}

	void move(char** lvl, const int cell_size, const int width, float& offset_y, int& hit_box_factor_x, int& hit_box_factor_y, int& Pheight, int& Pwidth)
	{
		int gridX = static_cast<int>(player_x) / cell_size;
		int gridY = static_cast<int>(player_y) / cell_size;
		...
		//FloatRect boundp = getSprite().getGlobalBounds();
		if (!active) return;
		Keyboard key;

		if (gridX >= 0 && gridX < width && gridY >= 0)
		{
			if (bottom_left == 'p' || bottom_center == 'p' || bottom_right == 'p')  // Note: gridY is first index for rows
			{
				setHurt(true);
				if (active && active->getBruh())  // Check active pointer
					setHurtRight(false);
				else
					setHurtRight(true);
			}
		...
		else
		{
			...
			if (active->getBruh())
			{
				if (active->getVelocityX() < 0)
				{
					active->setVelocityX(active->getVelocityX() + active->getAcceleration());
				}
			}
			else
			{
		...
		if (key.isKeyPressed(key.Space) && delayjump.getElapsedTime().asSeconds() > 0.3 && active->getOnGround() && (lvl[gridY][gridX] != 'w' || lvl[gridY][gridX] != 'q' || lvl[gridY][gridX] != 'e' || lvl[gridY][gridX] != 'b'))
		{
			{
	...
	void gettinghurt()
	{
		if ((!active->getCantHurtMe()) && hurt && hurtclock.getElapsedTime().asSeconds() > 3)
		{
			hp -= 0.5;
			hurtclock.restart();
			hurt = false;
			hurtright = false;
			active->setY(active->getY() - 60);
			cout << "IT HURT LIKE HELL" << endl;
			if (hurtright)
			{
				active->setX(active->getX() - 60);
			}
			else
			{
				active->setX(active->getX() + 60);
